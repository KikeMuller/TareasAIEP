...
class Articulo extends Model
{
    ...
    /**
     * Listar artículos de una sección usando el procedimiento almacenado.
     *
     * Llama a: CALL sp_articulos_por_seccion(p_seccion, p_limite)
     * El SP ejecuta el SELECT con LEFT JOIN a usuarios y devuelve
     * los artículos ordenados por destacado DESC y fecha DESC.
     *
     * Si el SP no está disponible, cae automáticamente al SELECT
    ...
    public function porSeccion(string $seccion, int $limite = 10): array
    {
        try {
            ...
            // CALL sp_articulos_por_seccion(p_seccion, p_limite)
            $stmt = $this->db->prepare(
                "CALL sp_articulos_por_seccion(:seccion, :limite)"
            );
            $stmt->bindValue(':seccion', $seccion, PDO::PARAM_STR);
            $stmt->bindValue(':limite',  $limite,  PDO::PARAM_INT);
            $stmt->execute();
            $articulos = $stmt->fetchAll();
            $stmt->closeCursor(); // Cerrar cursor tras procedimiento almacenado
            return $articulos;

        } catch (\PDOException $e) {
            // ── Fallback: SELECT directo si el SP no existe ──────────────
            // (por si stored_procedures.sql no fue ejecutado aún)
            $stmt = $this->db->prepare(
                "SELECT a.*, COALESCE(u.nombre, 'Redacción El Faro') AS autor_nombre
                 FROM {$this->table} a
